...
namespace CSharpCompiler
{
	...
	public class CustomDynamicDriver
	{
		...
		public bool Compile (out AssemblyBuilder outAssembly, AppDomain domain, bool generateInMemory)
		{
			...
			var output_file_name = Path.GetFileName(output_file);
			...
			if (!ctx.BuiltinTypes.CheckDefinitions(module)) return false;

			if (!assembly.Create(domain, AssemblyBuilderAccess.RunAndSave)) return false;
